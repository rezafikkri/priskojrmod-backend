...
import { editLicenseKeySchema, createLicenseKeySchema, licenseKeyIdSchema } from '../validators/license-key-validator';
...
export async function searchLicenseKeys({ key, select, limit }) {
  const session = await verifySession();
  if (!session) throw new Error('Unauthenticated');
