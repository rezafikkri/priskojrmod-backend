...
export async function searchLicenseKeys({ select, searchKey }) {
  const session = await verifySession();
  if (!session) throw new Error('Unauthenticated');

  const cacheFn = unstable_cache(async (select, searchKey) => {
    try {
      const licenseKeys = await pjmaDBPrismaClient.LicenseKey.findMany({
        select,
        where: {
  ...
  const licenseKeys = await cacheFn(select, searchKey);
  return JSON.parse(licenseKeys);
}
